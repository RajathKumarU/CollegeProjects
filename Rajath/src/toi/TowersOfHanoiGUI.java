...
public class TowersOfHanoiGUI extends Applet implements Runnable {
	...
	private static Color BACKGROUND_COLOR = new Color(155, 200, 200);
	private static Color BORDER_COLOR = new Color(100, 0, 0);
	private static Color DISK_COLOR = new Color(75, 100, 155);
	private static Color MOVE_DISK_COLOR = new Color(75, 100, 155);

	private Image OSC; // The off-screen canvas.

	private static final int GO = 1, SUSPEND = 2, TERMINATE = 3; // Values for
																	// status.

	private int status = GO, disk_count = 3; // Controls the execution of the
												// thread.

	private Thread runner; // A thread to run the animation.

	/*
	 * The following variables are the data needed for the animation. The three
	 * "piles" of disks are represented by the variables tower and towerHeight.
	 * towerHeight[i] is the number of disks on pile number i. For i=0,1,2 and
	 * for j=0,1,...,towerHeight[i]-1, tower[i][j] is an integer representing
	 * one of the ten disks. (The disks are numbered from 1 to 10.) (tower is
	 * null between repetitions of the solution.)
	 * 
	 * During the solution, as one disk is moved from one pile to another, the
	 * variable moveDisk is the number of the disk that is being moved, and
	 * moveTower is the number of the pile that it is currently on. This disk is
	 * not stored in the tower variable. it is drawn in a different color from
	 * the other disks.
	 * 
	 * All the data in these variables is for use in the drawCurrentFrame()
	 * method, which redraws the whole picture for each frame of the animation.
	 */
	...
	public void run() {
		...
		try {
			while (true) {
				...
				delay(2000);
				synchronized (this) {
					tower = new int[3][disk_count];
					for (int i = 0; i < disk_count; i++)
						tower[0][i] = disk_count - i;
					towerHeight = new int[3];
					towerHeight[0] = disk_count;
					if (OSC != null) {
						Graphics g = OSC.getGraphics();
						drawCurrentFrame(g);
						g.dispose();
	...
	private void putDisk(Color color, int disk, int t) {
		...
		if (OSC != null) {
			Graphics g = OSC.getGraphics();
			g.setColor(color);
			g.fillRoundRect(75 + 140 * t - 5 * disk - 5,
					116 - 12 * towerHeight[t], 10 * disk + 10, 10, 10, 10);
			g.dispose();
		}
		repaint();
	}
	...
	synchronized void drawCurrentFrame(Graphics g) {
		...
		for (int t = 0; t < 3; t++) {
			for (int i = 0; i < towerHeight[t]; i++) {
				int disk = tower[t][i];
				g.fillRoundRect(75 + 140 * t - 5 * disk - 5, 116 - 12 * i,
						10 * disk + 10, 10, 10, 10);
			}
		}
		if (moveDisk > 0) {
			g.setColor(MOVE_DISK_COLOR);
			g.fillRoundRect(75 + 140 * moveTower - 5 * moveDisk - 5,
					116 - 12 * towerHeight[moveTower], 10 * moveDisk + 10, 10,
					10, 10);
		}
	}
