...
public class TowerOfHanoi extends Applet implements ActionListener,
	...
	Button restartButton, helpButton, solveButton;

	Panel titlePanel, diffPanel, buttPanel, northPanel;

	Label diffLabel, titleLabel;

	JRadioButton threeDiscJRadio, fourDiscJRadio, fiveDiscJRadio,
			sixDiscJRadio;
	...
	public void init() {
		...
		solveButton = new Button("Solution");
		solveButton.setFont(new Font("", Font.BOLD, 16));
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == solveButton) {
			if (threeDiscJRadio.isSelected()) {
				solution.show(3);
			} else if (fourDiscJRadio.isSelected()) {
				solution.show(4);
			} else if (fiveDiscJRadio.isSelected()) {
				solution.show(5);
			} else if (sixDiscJRadio.isSelected()) {
				solution.show(6);
			}
		}
		...
		if (e.getSource() == threeDiscJRadio || e.getSource() == fourDiscJRadio
	...
	public void paint(Graphics g) {
		...
		if (arrowUp.getVisible() == true) {
			...
			g.fillPolygon(arrowUp.getPolyX(), arrowUp.getPolyY(),
					arrowUp.getPolyX().length);
			g.setColor(Color.black);
			g.drawPolygon(arrowUp.getPolyX(), arrowUp.getPolyY(),
					arrowUp.getPolyX().length);
		}

		if (arrowDown.getVisible() == true) {
			g.setColor(Color.red);
			g.fillPolygon(arrowDown.getPolyX(), arrowDown.getPolyY(),
					arrowDown.getPolyX().length);
			g.setColor(Color.black);
			g.drawPolygon(arrowDown.getPolyX(), arrowDown.getPolyY(),
					arrowDown.getPolyX().length);
		}
		...
		for (int i = 0; i <= 5; i++) {
			...
			if (disc[i].isUsed() == true) {
				...
				int discWidth, discDrawX, discDrawY;
	...
	public void updateArrows(int posX, int posY) {
		...
		if (holdingDisc == false) {
			...
			if (movedX > 50 && movedX < 650 && movedY > 160 && movedY < 400) {
				...
				// 2nd Tower
				if (movedX > 250 && movedX < 450
						&& arrowUp.getCurrentTower() != 1) {
					arrowUp.hand(1);
					repaint();
				}
	...
	public void updateDiscs(int posX, int posY) {
		...
		if (holdingDisc == true) {
			...
			// If mouse is within play area
			if (clickedX > 50 && clickedX < 650 && clickedY > 160
					&& clickedY < 400) {
			...
			if (tower[destTower].getLevel(i) == false) {
				...
				// If empty slot is above a disc
					...
					for (int j = 0; j <= 5; j++) {
						...
						/*
						 * If disc is used, located on the same tower, occupying
						 * the level below the current checking level, AND has
						 * the size BIGGER than the size of the current disc
						 * being drop
						 */
	...
	public void checkWinCondition() {
		...
		for (int i = 0; i <= 5; i++) {
			if (disc[i].isUsed() == true && disc[i].getTower() != 2) {
		...
		if (win == true) {
			JOptionPane.showMessageDialog(this, "You have won the game in "
					+ moveCount + " moves.", "Congratulations!",
	...
	public void pickupDisc(int sourceTower) {
		...
		for (int i = 0; i <= 5; i++) {
			if (disc[i].isUsed() == true && disc[i].getTower() == sourceTower) {
...
class ShowInstructions extends JFrame implements ActionListener {
	...
	public ShowInstructions() {
		...
		helpTextArea = new TextArea(instruction, 5, 50,
...
/**
 * This class shows the solution to the problem
 */
class Solution extends JFrame implements ActionListener {
	 ...
	 */
	public int tower(int N, int count, String s, String t, String d) {
		if (N == 1) {
			instruction += "Step-" + (count) + " -->  Move disc-" + N
					+ " from " + s + " to " + d + "\n";
			count++;
			return count;
		} else {
			count = tower(N - 1, count, s, d, t);
			instruction += "Step-" + (count) + " -->  Move disc-" + N
					+ " from " + s + " to " + d + "\n";
			count++;
			count = tower(N - 1, count, t, s, d);
			return count;
		}
	}
	 ...
	 * Method to show the help frame to the user
	 */
	public void show(int disks) {

		tower(disks, 1, "Tower-1", "Tower-2", "Tower3");

		helpFrame = new Frame("Optimal steps to solve the Problem");
		helpFrame.setLayout(new BorderLayout());
		...
		helpTextArea = new TextArea(instruction, 15, 50,
...
class Disc {
	...
	private boolean used, selected;

	private int size, onTower, onLevel;
...
class Arrow {
	...
	private int polyX[] = { 0, 0, 0, 0, 0, 0, 0 }, polyY[] = { 0, 0, 0, 0, 0,
			0, 0 }, dPolyX[] = { 0, 0, 0, 0, 0, 0, 0 }, dPolyY[] = { 0, 0, 0,
			0, 0, 0, 0 }, modX = 125, modY = 170, currentTower;
